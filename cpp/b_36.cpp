...
// Chuan hoa ho ten: viet hoa chu cai dau, con lai viet thuong
string chuanHoaHoTen(string s) {
    ...
    if(!res.empty()) res.pop_back(); // xoa khoang trang cuoi
    return res;
}

// Chuan hoa ngay sinh dd/mm/yyyy
string chuanHoaNgaySinh(string s) {
    if (s[1] == '/') s = "0" + s;          // ngay chi co 1 chu so, them 0
    if (s[4] == '/') s.insert(3, "0");     // thang chi co 1 chu so, them 0
    return s;
}

bool cmp(SinhVien a, SinhVien b) {
    return a.GPA > b.GPA; // sap xep giam dan theo diem GPA
}
